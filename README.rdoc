...
3. Create a YAML file <tt>config.yml</tt> structured as below <em>The database(s) should start with a "-" not a "*", some sites markup translate "-" into "*", so be aware that if your YAML file database list entries start with "*" and not "-" if may not work.</em> :
